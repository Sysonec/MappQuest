...
import dataCountries from './custom.geo.json' assert { type: 'json' };

// Import Antarctica as a standalone polygon
import antarctica from './antarctica.geo.json' assert { type: 'json' };
...
class GameState extends ToMapp {
  ...
  constructor() {
    ...
    // Choose difficulty
    gameOptions.addEventListener('click', this._chooseDiffuculty.bind(this));
    ...
    // Game details btns
    buttonsContainer.addEventListener('click', this._handleClicks.bind(this));
  ...
  _playMusic() {
    ...
    this.#audio.volume = JSON.parse(window.localStorage.getItem('audioVolume'));
  ...
  _openModal(modalName) {
    ...
    const calcSavedtime = () => (savedTime ? this._getSavedTime() : ['', '']);
  ...
  _setMarker(e) {
    ...
    const markerNameWarning = document.querySelector('.marker-name-warning');
  ...
  _displayHighscore() {
    ...
    highscoreGame.textContent = highscoreSaved === null ? 0 : highscoreSaved;
  }
  ...
  _settingsControl(e) {
    ...
    this.#audioEffectsSlider = document.querySelector('.audio-effects-input');
    ...
    const switchZoomBtn = document.querySelector('.zoom-controls-switch-btn');
    ...
    if (
      ...
      // Change switchAudioBtn text and icon when muted
      setSwitchStyle(switchAudioBtn, 'Sound off', 'fa-solid fa-volume-xmark');
    }
    ...
    if (
      ...
      setSwitchStyle(switchZoomBtn, 'Zoom off', 'fa-solid fa-magnifying-glass');
  ...
  _playAgain(e) {
    ...
    const playAgainBtnPresent = e.target.classList.contains('play-again-btn');
  ...
  _setTimer(e) {
    ...
    const checkInputType = () => {
      timerLeft.innerHTML = `<i class="fa-regular fa-clock"></i> ${this.#min}:${
        this.#sec
      }`;
    ...
    if (checkForEl('save-time')) {
      ...
      window.localStorage.setItem('savedTime', JSON.stringify(this.#savedTime));
    }
  ...
  _handleClicks(e) {
    ...
    if (btn.classList.contains('settings-btn')) {
      ...
      const sliderEffectsEl = document.querySelector('.audio-effects-input');
    ...
    if (btn.classList.contains('est-position-btn')) {
      ...
      const countryCoordsRandom = [
        this.#randomCountryDetails.properties.label_y + Math.random() * numY,
        this.#randomCountryDetails.properties.label_x + Math.random() * numX,
      ];
